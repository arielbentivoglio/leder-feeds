...
(function () {
  ...
  function injectStyles(set) {
    ...
    var css = "" +
      ...
      ".ldr-pa-shadow{filter:blur(9px);pointer-events:none;opacity:.85;}" +
      ".ldr-pa-rug{pointer-events:none;filter:drop-shadow(0 1px 1px rgba(0,0,0,.22));}" +
  ...
  function buildModal(set) {
    ...
    modal.innerHTML =
      ...
      '<div class="ldr-pa-stage">' +
        ...
        '<canvas class="ldr-pa-shadow"></canvas>' +
    ...
    var shadow = modal.querySelector(".ldr-pa-shadow"); // canvas: copia el alpha de "rug" -> sombra con silueta real
    ...
    shadow.width = baseW; shadow.height = baseH;
    shadow.style.width = baseW + "px";
    shadow.style.height = baseH + "px";

    // ─── Luz ambiente + sombra con silueta real ──────────────────────────────
    // ambientFactor ajusta brillo/saturacion de la alfombra segun que tan
    // clara/oscura es la foto del ambiente, para que no se vea "pegada".
    // 190 = luminancia tipica de una foto de producto bien iluminada, se usa
    // como referencia neutra (factor 1 = sin cambios).
    var ambientFactor = 1;

    function drawCoverImage(ctx, img, dw, dh) {
      // Replica el object-fit:cover del <img> para samplear los mismos
      // pixeles que el cliente ve en pantalla.
    ...
    function calcularAmbientFactor() {
      try {
        var rect = stage.getBoundingClientRect();
        var w = Math.round(rect.width) || 400, h = Math.round(rect.height) || 300;
        var tmp = document.createElement("canvas");
        tmp.width = w; tmp.height = h;
        var tctx = tmp.getContext("2d");
        drawCoverImage(tctx, roomImg, w, h);
        var data = tctx.getImageData(0, 0, w, h).data;
        var total = 0, n = 0;
        for (var i = 0; i < data.length; i += 4 * 9) { // muestreo cada 9 px, no hace falta recorrer todo
          total += 0.299 * data[i] + 0.587 * data[i + 1] + 0.114 * data[i + 2];
          n++;
        }
        var brillo = n ? total / n : 190;
        ambientFactor = Math.min(1.25, Math.max(0.6, brillo / 190));
      } catch (e) {
        ambientFactor = 1; // si algo falla (foto no decodificada todavia, etc.) queda sin ajuste
      }
    }

    function dibujarSombra() {
      ctxShadow.clearRect(0, 0, baseW, baseH);
      ctxShadow.drawImage(rug, 0, 0); // copia el alpha ya dibujado en "rug"
      ctxShadow.globalCompositeOperation = "source-in";
      ctxShadow.fillStyle = "rgba(15,10,5,0.42)";
      ctxShadow.fillRect(0, 0, baseW, baseH);
    ...
    function aplicarImagenActual() {
      ...
      tmp.onload = function () {
        ...
        ctxRug.filter = "brightness(" + ambientFactor.toFixed(2) + ") saturate(0.94)";
